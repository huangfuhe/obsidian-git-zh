<script lang="ts">
    ...
    function getCommitActionDescription(sync: boolean): string {
        const suffix = sync ? "并同步" : "";
        if (hasConflicts) {
            return "解决冲突后再提交";
        }
        if (!hasChanges) {
            return sync ? "同步（没有需要提交的变更）" : "没有需要提交的变更";
        }
        if (sync) {
            return "提交全部变更并同步";
        }
        if (stagedCount > 0) {
            return `提交 ${stagedCount} 个已暂存文件${suffix}`;
        }
        if (!plugin.settings.autoStageOnEmptyIndex) {
            return "没有已暂存内容，请先暂存变更再提交";
        }
        return `暂存并提交全部 ${changedCount} 个变更文件${suffix}`;
    }
