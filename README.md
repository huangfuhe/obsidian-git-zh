...
这是 [Vinzent03/obsidian-git](https://github.com/Vinzent03/obsidian-git) 的非官方中文维护版，保留原插件的 Git、自动提交、拉取、推送、历史和差异视图功能，并将用户可见界面翻译为中文。

本项目基于原项目的 MIT License 发布。原作者和许可证信息保留在仓库中。
...
1. 安装并启用 `BRAT` 插件。
2. 执行 `BRAT: Add Beta plugin`。
3. 输入本仓库地址：`https://github.com/huangfuhe/obsidian-git-zh`。
4. 在社区插件设置中启用 `Git 中文版`。

安装中文版本后，请停用原版 `Git` 插件，避免两个插件同时操作同一个仓库。

### 手动安装

从 Releases 下载 `main.js`、`manifest.json` 和 `styles.css`，放入 Vault 的 `.obsidian/plugins/obsidian-git-zh/` 目录，然后在 Obsidian 中启用插件。

## 说明

- 这是非官方中文维护版，功能代码来自上游项目。
- 上游功能更新后，本项目需要同步并重新翻译。
- GitHub 认证信息请使用 SSH 或细粒度 Personal Access Token，不要把密钥写入笔记或提交到仓库。
